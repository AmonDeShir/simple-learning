...
import { DefaultLearningProgress } from '../schemas/learning-progress.schema';
...
import { getDayOfNextMonth } from '../utils/get-days-in-current-month';
...
export namespace WordDao {
  ...
  export async function findByIds(ids: string[]): Promise<WordWithId[]> {
    return await dao.find({ _id: { $in: ids.map(id => new Types.ObjectId(id)) }});
  }
  ...
  export async function clear() {
    ...
    let unusedWords: WordDocument[] = await dao.find({ _id: { $nin: inUse }});

    await dao.deleteMany({ _id: { $in: unusedWords.map(({ _id }) => _id) }})
  }
