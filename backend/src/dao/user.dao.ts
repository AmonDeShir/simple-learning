...
import { locallyUserModel } from '../models/locally-user.model';
...
export namespace UsersDao {
  ...
  export const locallyModel = locallyUserModel;
  ...
  export async function findAnyTypeUserById(id: string | ObjectId): Promise<(User & LocallyUser & Document<any, any, any> & { _id: Types.ObjectId; }) | null> {
    ...
    if (!result) {
      return await locallyModel.findById(id);
    }
  ...
  export async function findAll() {
    return [...(await model.find()), ... await(locallyModel.find())];
  }
}