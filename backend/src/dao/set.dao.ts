...
import { WordWithId } from '../types/word.type';
...
export namespace SetDao {
  ...
  export async function getLastTen(id: string): Promise<SetDocument[]> {
    return await model.find({ user: id }).sort({ lastLearn: -1 }).limit(10);
  }

  export async function searchByWordId(id: string): Promise<SetDocument[]> {
    const sets = await model.find({ words: id });

    return sets;
  }

  export async function search(query: string, user: string) {
    const result: SetDocument[] = await model.find({ title: { $regex: query, $options: 'i' }, user });
